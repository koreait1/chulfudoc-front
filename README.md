...
**회원 인증/관리 서비스**
- 회원 가입
n 회원 가입 시 필수 입력 항목 체크
n 중복(아이디, 이메일), 패스워드 보안 레벨, 이메일 형식, 휴대전화 번호 형식 검증
n REST API를 활용한 소셜 연동 회원 가입
-      로그인
n 로그인 필수 항목 정보 전달 -> BE에서 회원 존재 여부 체크 -> JWT 토큰 발급
n 소셜 계정 : 소셜 토큰 체크 -> JWT 토큰 발급
-      본인 인증(이메일)
n 이메일 형식 체크 후 이메일 발송(템플릿 형태) 요청
n Redis 내 인증번호 임시 저장(BE) -> 입력한 인증번호와 일치 여부 체크
메인페이지
-      홈페이지 컨셉에 맞는 스타일링
-      UI 기획 및 제작
