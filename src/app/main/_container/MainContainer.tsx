...
const MainContainer = () => {
  ...
  const sections = [
    <Section key={0}>
      ...
      <Headers isOpen={profileOpen} setIsOpen={setProfileOpen} />
      <PageMain>
        {webcamAble ? (
          <DetectContainer
            webcamAble={webcamAble}
            onChange={(item) => setWebcamAble(item)}
          />
        ) : (
          <>
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      if (isScrolling || popUpOpen || profileOpen) return
      isScrolling = true
      if (e.deltaY > 0) {
  ...
  }, [sections.length, popUpOpen, profileOpen])
